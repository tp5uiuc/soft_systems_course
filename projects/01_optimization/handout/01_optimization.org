...
:END:
  ...
  simple ~numpy npz~ files with the knapsack capacity, number of different
  items (which are all assumed to be distinct so you do not need to worry about
  /choosing/ same items), and their values + weights. For
  convenience you can read the information using ~numpy~'s ~load~ function,
