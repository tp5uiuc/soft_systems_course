...
\label{sec:orgb59a6f7}
\begin{frame}[label={sec:org7905d11},fragile]{Additional packages}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org835b95a}]{Quick review}
...
\label{sec:org8db55b4}
\begin{frame}[label={sec:org56fd10c}]{The algorithm}
...
\begin{frame}[label={sec:org9a8db65}]{The algorithm contd.}
...
\begin{frame}[label={sec:org69cd8ed}]{The algorithm contd..}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb909125}]{Starting CMAes}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org5eac0ef}]{Starting CMAes: more on problem dependent parameters}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org34b1412},fragile]{First step : Sampling}
...
\end{frame}
\begin{frame}[label={sec:org708c15c},fragile]{Sampling : Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org4ac8f75}]{Normal Distribution\footnote{\href{https://upload.wikimedia.org/wikipedia/commons/2/25/The\_Normal\_Distribution.svg}{ Wikimedia Commons}}}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org89012d1}]{Covariance Matrix \footnote{\href{https://www.slideshare.net/OsamaSalaheldin2/cmaes-presentation}{CMAes overview, Slideshare}}}
\begin{columns}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org75ac60d}]{Covariance Matrix}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{block}{\(\mathcal{N}(\gv{0}, \bv{I})\)}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org379b7bb}]{Covariance Matrix : Example 1}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org30bbbc6}]{Covariance Matrix : Example 2}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org9220f57}]{Covariance Matrix : Example 3}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgfb063a0}]{Covariance Matrix : Example 4}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org8380689}]{Covariance Matrix : Definition}
...
\begin{frame}[label={sec:orgf43ca46}]{Variance}
...
\begin{frame}[label={sec:orgaff27a9}]{Variance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org13b8807}]{Covariance}
...
\begin{frame}[label={sec:org9ad5bb5}]{Covariance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgbc6f20f}]{Covariance Matrix : Definition}
Covariance Matrix decides the ``direction'' of the population\ldots{}
...
\begin{frame}[label={sec:orgf719847},fragile]{Sampling : Python--Answers}
 \begin{itemize}
\item What is \(\bv{C}\) / \texttt{cov} (in a 2D case) and its meaning?
\begin{enumerate}
...
\begin{frame}[label={sec:orgbbb563d}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgae2f9f8}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math--Answers}
\begin{itemize}
\item How does CMA estimate \(\bv{C}\)?
\begin{enumerate}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgef36e77}]{PCA}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org84e0154}]{Sampling : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2c0d31d},fragile]{Second step : Selection}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7b0f0dc}]{Selection : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb6f0aaf},fragile]{Third step : Recombination}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org3e7a1a0}]{Third step : Recombination--Answers}
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org2c1342c},fragile]{Fourth step : Step size control}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org4b4ba93},fragile]{Step size control: Computing/Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgd826f8c}]{Step size control \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgc6d5387}]{Step size : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org39358ab},fragile]{Fifth step : Covariance matrix adaptation}
...
\begin{frame}[label={sec:org7afeebc}]{CMA \(\rightarrow\) math}
...
\begin{frame}[label={sec:org1d20298}]{CMA: Parameters}
...
\begin{frame}[label={sec:orgecdb9f6},fragile]{Terminating CMA}
...
\begin{frame}[label={sec:orgfbe4a74},fragile]{Terminating CMA contd.}
 \begin{block}{Problem independent}
\begin{itemize}
...
\begin{frame}[label={sec:orgafd2ee2}]{Boundaries/Constraints in CMA : Best solution strictly inside}
\begin{itemize}
...
\begin{frame}[label={sec:orgf23a697}]{Boundaries/Constraints in CMA : Repair}
\begin{itemize}
...
\label{sec:orgc3744cb}
\begin{frame}[label={sec:orgc06c97c}]{CMAes vs GA--setup}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgcec4b6d}]{CMAes vs GA--setup}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd687617}]{Comparison between functions\footnote{\href{http://blog.otoro.net/2017/10/29/visual-evolution-strategies/}{Otoro}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{frame}
\begin{frame}[label={sec:org715b980}]{Simple ES}
\begin{block}{Scheme}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org1830d9f}]{Simple ES-Observations}
...
\begin{frame}[label={sec:org7a85582}]{Simple ES-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:org98627fd}]{Simple GA}
\begin{block}{Scheme}
\begin{itemize}
...
\begin{frame}[label={sec:orgad350aa}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
\item
...
\begin{frame}[label={sec:org46f14e7}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:orgde69e39}]{CMAes-Observations}
...
\begin{frame}[label={sec:orgb8995bc}]{CMAes-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9a859e0}]{CMAes-Some interesting videos}
\begin{itemize}
