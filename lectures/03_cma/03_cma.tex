...
\label{sec:org96c0356}
\begin{frame}[label={sec:orgacc808a},fragile]{Additional packages}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0488cba}]{Quick review}
...
\begin{frame}[label={sec:org1131480}]{The algorithm contd..}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga779175}]{Starting CMAes}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org3879f81}]{Starting CMAes: more on problem dependent parameters}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org9b76091},fragile]{First step : Sampling}
...
\end{frame}
\begin{frame}[label={sec:orgd7e438b},fragile]{Sampling : Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgdf282d3}]{Covariance Matrix \footnote{\href{https://www.slideshare.net/OsamaSalaheldin2/cmaes-presentation}{CMAes overview, Slideshare}}}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org89f7652}]{Covariance Matrix}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{block}{\(\mathcal{N}(\gv{0}, \bv{I})\)}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orga237feb}]{Covariance Matrix : Example 1}
...
\begin{center}
	\begin{tikzpicture}[baseline,scale=0.6]
		...
		\draw [black!50] (axis cs:-2.5,0,0) -- (axis cs:2.5,0,0);
		\draw [black!50] (axis cs:0,-2.5,0) -- (axis cs:0,2.5,0);
		\draw [thick,->] (axsi cs:0.2,0) arc (0:45:0.2) node [midway, above right]{tr};
		\end{axis}
	\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgfca767f}]{Covariance Matrix : Example 2}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org0a4001a}]{Covariance Matrix : Example 3}
...
\begin{frame}[label={sec:org82b1451}]{Variance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org747081b}]{Covariance}
...
\begin{frame}[label={sec:orge570917}]{Covariance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org4f41a66}]{Covariance Matrix : Definition}
Covariance Matrix decides the ``direction'' of the population\ldots{}
...
\begin{frame}[label={sec:org472d801},fragile]{Sampling : Python--Answers}
 \begin{itemize}
\item What is \(\bv{C}\) / \texttt{cov} (in a 2D case) and its meaning?
\begin{enumerate}
...
\begin{frame}[label={sec:orgb864790}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org987100e}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math--Answers}
\begin{itemize}
\item How does CMA estimate \(\bv{C}\)?
\begin{enumerate}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org443292d}]{PCA}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdc97072}]{Sampling : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgaaf2b3f},fragile]{Second step : Selection}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org473f9fb}]{Selection : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org339992f},fragile]{Third step : Recombination}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgc89144a}]{Third step : Recombination--Answers}
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org50b797a},fragile]{Fourth step : Step size control}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org27e0d9e},fragile]{Step size control: Computing/Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgebd9694}]{Step size control \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org945c215}]{Step size : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgcb66afc},fragile]{Fifth step : Covariance matrix adaptation}
...
\begin{frame}[label={sec:org104b60b}]{CMA \(\rightarrow\) math}
...
\begin{frame}[label={sec:orgc9ba71a}]{CMA: Parameters}
...
\begin{frame}[label={sec:org589796b},fragile]{Terminating CMA}
...
\begin{frame}[label={sec:org2d1cef3},fragile]{Terminating CMA contd.}
 \begin{block}{Problem independent}
\begin{itemize}
...
\begin{frame}[label={sec:orgdea1ab2}]{Boundaries/Constraints in CMA : Best solution strictly inside}
\begin{itemize}
...
\begin{frame}[label={sec:orged789ad}]{Boundaries/Constraints in CMA : Repair}
\begin{itemize}
...
\label{sec:org85e49bc}
\begin{frame}[label={sec:org1328448}]{CMAes vs GA--setup}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga415c0d}]{CMAes vs GA--setup}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgef2d782}]{Comparison between functions\footnote{\href{http://blog.otoro.net/2017/10/29/visual-evolution-strategies/}{Otoro}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{frame}
\begin{frame}[label={sec:org7196ffe}]{Simple ES}
\begin{block}{Scheme}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge413457}]{Simple ES-Observations}
...
\begin{frame}[label={sec:org55739b2}]{Simple ES-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:orgfc5d597}]{Simple GA}
\begin{block}{Scheme}
\begin{itemize}
...
\begin{frame}[label={sec:org7adc9c1}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
\item
...
\begin{frame}[label={sec:orgc84b23a}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:org7431a24}]{CMAes-Observations}
...
\begin{frame}[label={sec:org4e055f9}]{CMAes-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge338b9e}]{CMAes-Some interesting videos}
\begin{itemize}
