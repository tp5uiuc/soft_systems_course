...
\begin{frame}[label={sec:orgf9d6813}]{The algorithm contd..}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org3a3d244}]{Starting CMAes}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2b3e700}]{Starting CMAes: more on problem dependent parameters}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0fa6b4c},fragile]{First step : Sampling}
...
\end{frame}
\begin{frame}[label={sec:org60266f8},fragile]{Sampling : Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org0caf30c}]{Normal Distribution\footnote{\href{https://upload.wikimedia.org/wikipedia/commons/2/25/The\_Normal\_Distribution.svg}{ Wikimedia Commons}}}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:orgf23b99f}]{Covariance Matrix \footnote{\href{https://www.slideshare.net/OsamaSalaheldin2/cmaes-presentation}{CMAes overview, Slideshare}}}
\begin{columns}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org460cdb8}]{Covariance Matrix}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{block}{\(\mathcal{N}(\gv{0}, \bv{I})\)}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd9836b7}]{Covariance Matrix : Example 1}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd4ae6e8}]{Covariance Matrix : Example 2}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org2803d70}]{Covariance Matrix : Example 3}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org0b57d70}]{Covariance Matrix : Example 4}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgb3cb799}]{Covariance Matrix : Definition}
...
\begin{frame}[label={sec:org0e64d9c}]{Variance}
...
\begin{frame}[label={sec:orgda01d28}]{Variance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org958f425}]{Covariance}
...
\begin{frame}[label={sec:org329c6c5}]{Covariance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org58d0407}]{Covariance Matrix : Definition}
Covariance Matrix decides the ``direction'' of the population\ldots{}
...
\begin{frame}[label={sec:orgca7b305},fragile]{Sampling : Python--Answers}
 \begin{itemize}
\item What is \(\bv{C}\) / \texttt{cov} (in a 2D case) and its meaning?
\begin{enumerate}
...
\begin{frame}[label={sec:org16c9e2b}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org96b84a0}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math--Answers}
\begin{itemize}
\item How does CMA estimate \(\bv{C}\)?
\begin{enumerate}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org66e151c}]{PCA}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org775a3e0}]{Sampling : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org63c2903},fragile]{Second step : Selection}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgfa6f49d}]{Selection : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org03f4a94},fragile]{Third step : Recombination}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org4a2d2cd}]{Third step : Recombination--Answers}
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org7e3287a},fragile]{Fourth step : Step size control}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgcb9e525},fragile]{Step size control: Computing/Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgb641331}]{Step size control \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org6ea6db4}]{Step size : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org052110f},fragile]{Fifth step : Covariance matrix adaptation}
...
\begin{frame}[label={sec:org8158935}]{CMA \(\rightarrow\) math}
...
\begin{frame}[label={sec:org22d10ff}]{CMA: Parameters}
...
\begin{frame}[label={sec:orgb8bde32},fragile]{Terminating CMA}
...
\begin{frame}[label={sec:orgf344537},fragile]{Terminating CMA contd.}
 \begin{block}{Problem independent}
\begin{itemize}
...
\begin{frame}[label={sec:org1dc5f9a}]{Boundaries/Constraints in CMA : Best solution strictly inside}
\begin{itemize}
...
\begin{frame}[label={sec:org610168c}]{Boundaries/Constraints in CMA : Repair}
\begin{itemize}
...
\label{sec:org2dfe477}
\begin{frame}[label={sec:org692c0a8}]{Brachistochrone problem\footnote{\href{https://en.wikipedia.org/wiki/Johann\_Bernoulli}{Johann Bernoulli}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org5924a52}]{Brachistochrone problem}
\begin{columns}
...
\begin{frame}[label={sec:org28c53fc}]{Brachistochrone problem : more formally}
\begin{center}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org378ff6c}]{Representation}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd8deaf3}]{Representation}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge9f5029}]{Fitness function}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd8453b8}]{Constraints and penalties}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8a6f81b}]{Aliters}
\begin{columns}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org381605a}]{More history\footnote{\href{https://en.wikipedia.org/wiki/Brachistochrone\_curve}{Brachistochrone curve wiki}}}
\footnotesize
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2b0ac61}]{Dido's isoperimetric problem\footnote{\href{https://mathematicalgarden.wordpress.com/2008/12/21/the-problem-of-dido/}{Mathematical Garden}}}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga9bf06d}]{Results}
\begin{itemize}
...
\label{sec:orgbc8d819}
\begin{frame}[label={sec:org8be6a06}]{CMAes vs GA--setup}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd5dd6ff}]{CMAes vs GA--setup}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org62ddf02}]{Comparison between functions\footnote{\href{http://blog.otoro.net/2017/10/29/visual-evolution-strategies/}{Otoro}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{frame}
\begin{frame}[label={sec:orgb1c6b69}]{Simple ES}
\begin{block}{Scheme}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc80a2cb}]{Simple ES-Observations}
...
\begin{frame}[label={sec:orgf20c6fd}]{Simple ES-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:orgd77fabd}]{Simple GA}
\begin{block}{Scheme}
\begin{itemize}
...
\begin{frame}[label={sec:org1afc628}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
\item
...
\begin{frame}[label={sec:orge1f678f}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:orge5377b2}]{CMAes-Observations}
...
\begin{frame}[label={sec:org0c71fe3}]{CMAes-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdd045c3}]{CMAes-Some interesting videos}
\begin{itemize}
