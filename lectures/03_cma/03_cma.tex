...
\label{sec:org37131bf}
\begin{frame}[label={sec:org50a5b5c},fragile]{Additional packages}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc16e946}]{Quick review}
...
\begin{frame}[label={sec:orgc43e55b}]{The algorithm contd..}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org983a302}]{Starting CMAes}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb94ef00}]{Starting CMAes: more on problem dependent parameters}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org095d970},fragile]{First step : Sampling}
...
\end{frame}
\begin{frame}[label={sec:orga70e200},fragile]{Sampling : Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org6ffafa2}]{Normal Distribution\footnote{\href{https://upload.wikimedia.org/wikipedia/commons/2/25/The\_Normal\_Distribution.svg}{ Wikimedia Commons}}}
\begin{figure}[htbp]
\centering
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:orgb1c65b0}]{Covariance Matrix \footnote{\href{https://www.slideshare.net/OsamaSalaheldin2/cmaes-presentation}{CMAes overview, Slideshare}}}
\begin{columns}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org0f57920}]{Covariance Matrix}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{block}{\(\mathcal{N}(\gv{0}, \bv{I})\)}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd061f3f}]{Covariance Matrix : Example 1}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org7e37eab}]{Covariance Matrix : Example 2}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org8cca46a}]{Covariance Matrix : Example 3}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org9b75f20}]{Covariance Matrix : Example 4}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgd2020a5}]{Covariance Matrix : Definition}
...
\begin{frame}[label={sec:org7200999}]{Variance}
...
\begin{frame}[label={sec:org401179b}]{Variance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgc60b5a5}]{Covariance}
...
\begin{frame}[label={sec:orgdd3006c}]{Covariance}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org72dce17}]{Covariance Matrix : Definition}
Covariance Matrix decides the ``direction'' of the population\ldots{}
...
\begin{frame}[label={sec:org32001e6},fragile]{Sampling : Python--Answers}
 \begin{itemize}
\item What is \(\bv{C}\) / \texttt{cov} (in a 2D case) and its meaning?
\begin{enumerate}
...
\begin{frame}[label={sec:orgac07d5c}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org8a6e493}]{Sampling : Idea of \(\bv{C}\) \(\rightarrow\) math--Answers}
\begin{itemize}
\item How does CMA estimate \(\bv{C}\)?
\begin{enumerate}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org1c67bb7}]{PCA}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orged4eec8}]{Sampling : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org03d422a},fragile]{Second step : Selection}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org91d5f1e}]{Selection : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgee7af76},fragile]{Third step : Recombination}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:orgf6f268b}]{Third step : Recombination--Answers}
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org592f900},fragile]{Fourth step : Step size control}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org706d58a},fragile]{Step size control: Computing/Python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org6722831}]{Step size control \(\rightarrow\) math}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org69dccf0}]{Step size : Parameters}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgec4d930},fragile]{Fifth step : Covariance matrix adaptation}
...
\begin{frame}[label={sec:orga12200c}]{CMA \(\rightarrow\) math}
...
\begin{frame}[label={sec:org6dee519}]{CMA: Parameters}
...
\begin{frame}[label={sec:org008166e},fragile]{Terminating CMA}
...
\begin{frame}[label={sec:org64e85f3},fragile]{Terminating CMA contd.}
 \begin{block}{Problem independent}
\begin{itemize}
...
\begin{frame}[label={sec:org661b037}]{Boundaries/Constraints in CMA : Best solution strictly inside}
\begin{itemize}
...
\begin{frame}[label={sec:orgc71ec6a}]{Boundaries/Constraints in CMA : Repair}
\begin{itemize}
...
\label{sec:org9821edb}
\begin{frame}[label={sec:org411f296}]{Brachistochrone problem}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgf196be8}]{Brachistochrone problem}
\begin{columns}
...
\begin{frame}[label={sec:orgf745c8e}]{Brachistochrone problem : more formally}
\begin{center}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org4d9ab0f}]{Representation}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org2034796}]{Representation}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc1e121b}]{Fitness function}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgf670444}]{Constraints and penalties}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2f4be64}]{Aliters}
\begin{columns}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6349018}]{More history\footnote{\href{https://en.wikipedia.org/wiki/Brachistochrone\_curve}{Brachistochrone curve wiki}}}
\footnotesize
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgdcd614d}]{Dido's isoperimetric problem\footnote{\href{https://mathematicalgarden.wordpress.com/2008/12/21/the-problem-of-dido/}{Mathematical Garden}}}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org35668f2}]{Results}
\begin{itemize}
...
\label{sec:org031a09c}
\begin{frame}[label={sec:orgf237c2a}]{CMAes vs GA--setup}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org398361e}]{CMAes vs GA--setup}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3bfbf80}]{Comparison between functions\footnote{\href{http://blog.otoro.net/2017/10/29/visual-evolution-strategies/}{Otoro}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\end{frame}
\begin{frame}[label={sec:org86e4477}]{Simple ES}
\begin{block}{Scheme}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org28f8584}]{Simple ES-Observations}
...
\begin{frame}[label={sec:org05da07c}]{Simple ES-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:org7b13fb4}]{Simple GA}
\begin{block}{Scheme}
\begin{itemize}
...
\begin{frame}[label={sec:org8cfc7f8}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
\item
...
\begin{frame}[label={sec:orge6d6c07}]{Simple GA-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\begin{frame}[label={sec:orgfb9170a}]{CMAes-Observations}
...
\begin{frame}[label={sec:org72fcc66}]{CMAes-Observations}
\begin{block}{Convergence}
\begin{itemize}
\item What do you expect for general problems?
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2193c5c}]{CMAes-Some interesting videos}
\begin{itemize}
