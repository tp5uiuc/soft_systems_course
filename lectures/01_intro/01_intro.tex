...
\begin{frame}[label={sec:org783dd2e}]{Logistics}
\begin{itemize}
\item Office hours
...
\label{sec:org3338756}
\begin{frame}[label={sec:orgc2120f2}]{Last class}
\begin{columns}
...
\begin{frame}[label={sec:orgef6bf12},fragile]{Content}
 \begin{itemize}
...
\begin{frame}[label={sec:org45bfd66},fragile]{Quick survey}
 \begin{itemize}
\item \texttt{Python}?
...
\end{frame}
\section{Why python?}
\label{sec:org99c99f5}
\begin{frame}[label={sec:org178eb31}]{Python is popular \footnote{Slide credits: \href{https://github.com/williamgrimes/teach\_python\_in\_notebooks/blob/master/python\_course.pdf}{William Grimes @ Github}}}
\begin{columns}
...
\begin{frame}[label={sec:orgc07a09a}]{Python as a language}
\begin{itemize}
...
\item Open source packages\footnote{\href{https://pypi.org/}{PyPi}} (163,402 as of January 1, 2019)
\item Widely used as an application/library API\footnote{\href{https://www.pluralsight.com/blog/software-development/why-python}{Pluralsight}\label{org34129d2}}
\item \(\therefore\) productivity
\end{itemize}
\end{frame}

\begin{frame}[label={sec:org4d0e777},fragile]{Interactive\footnote{\href{https://wiki.python.org/moin/IntegratingPythonWithOtherLanguages}{Wiki Python}}}
 \begin{itemize}
...
\begin{frame}[label={sec:org7e98073}]{FlappyBird      \footnote{\href{https://yanpanlau.github.io/2016/07/10/FlappyBird-Keras.html}{ FlappyBird-Keras}, \href{https://github.com/yanpanlau/Keras-FlappyBird/blob/master/qlearn.py}{Code} , \href{https://www.pygame.org/wiki/about}{Pygame}, \href{https://scikit-image.org/}{Scikit-image}, \href{https://www.tensorflow.org/}{TensorFlow}}}
\begin{columns}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{images/dot_success.pdf}
\caption{Workflow}
\end{figure}
\end{column}
\end{columns}
...
\begin{frame}[label={sec:org24ce6d6},fragile]{Comp. modeling with HPC using FOSS packages}
...
\begin{frame}[label={sec:org4841e6e}]{Your favorite applications use Python! \textsuperscript{\cref{org34129d2}}}
\begin{itemize}
\item Dropbox
\item Spotify
\item Email clients
...
\begin{frame}[label={sec:orgc44da0c},fragile]{We will use python for\footnote{\href{https://zulko.github.io/blog/2014/11/13/things-you-can-do-with-python-and-pov-ray/\#disqus\_thread}{Vapory/Povray}}\textsuperscript{,}\,\footnote{\href{http://mattia-lab.com/work/soft-filaments/}{Mattialab}}}
 \begin{itemize}
...
\label{sec:org27a5c7c}
\begin{frame}[label={sec:org0876b6d}]{Shell?}
\begin{itemize}
...
\begin{frame}[label={sec:orge4b700f},fragile]{On to your shell}
 \begin{itemize}
...
\begin{frame}[label={sec:orgbfd5224},fragile]{Shell demo}
...
\begin{frame}[label={sec:orge8fd7f2},fragile]{View current directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org83914c6},fragile]{My current directory structure?}
 \begin{block}{Command}
\begin{minted}[]{fish}
...
\begin{frame}[label={sec:org6669210},fragile]{What's my current directory?}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org7c5a394},fragile]{Navigate to another directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org2604a5e},fragile]{Navigate back to source directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
cd images # Change Directory
...
\begin{frame}[label={sec:org55b2316},fragile]{Make a new directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org526276c},fragile]{Remove directories and files}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org0443804},fragile]{If the directory has files, then}
 \begin{block}{Command}
\begin{minted}[]{bash}
mkdir MyFolderName
...
\begin{frame}[label={sec:org9f07ecf},fragile]{See file contents}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:orgbd63e69},fragile]{Move files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:orgbe25033},fragile]{Copy files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org50b5525},fragile]{Launch applications}
 \begin{block}{Command}
\begin{minted}[]{bash}
...
\begin{frame}[label={sec:orge4d768a}]{More about your shell}
\begin{itemize}
...
\label{sec:orgcdd7072}
\begin{frame}[label={sec:org3864610},fragile]{Outline\footnote{\url{https://www.python-course.eu/python3\_course.php}}}
 \begin{itemize}
...
\begin{frame}[label={sec:org68faed1},fragile]{Installation/Setup}
 \begin{itemize}
\item Fetch the install instructions at Compass (Setup -> 00\_setup\_basics.pdf) and follow it.
\item Even a VirtualMachine (the OS agnostic step in the install instructions) is
...
\begin{frame}[label={sec:org179b75d},fragile]{Python interactive shell}
 \note{:B\_note:
\begin{itemize}
...
\begin{itemize}
\item Calculation::inexact arithmetic--Python: 0.1+0.2 = 0.3? Show matlab also has the same issue.

\item Python 3: 1/3 = 0.3333. Switch to Python2 gotcha. Show that 1/3 is 0;
\item Addition of strings ``hello'' + ``wprld''
\item Multiplication of strings as concatenation ``hello''*4
\end{itemize}}
\alert{Demo}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgac40276},fragile]{Python scripts}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org49f05a8},fragile]{Common types and operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org72af865},fragile]{Common types and operations contd.}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org6bdda87},fragile]{Some more operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgdc652ec},fragile]{Practice}
...
\begin{frame}[label={sec:org7c1733d},fragile]{Sequential types}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb19e3ec},fragile]{Lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org516c41a},fragile]{Examples of lists}
 \begin{center}
\begin{tabular}{ll}
\toprule
List & Description\\
...
\begin{frame}[label={sec:orgb6d9a68},fragile]{Operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org7700782},fragile]{Additional operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0de9d24},fragile]{Activity}
 \note{:B\_note:
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgede8f86},fragile]{Tuples (very briefly)}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org95d8f88},fragile]{Copies of lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org584b70f},fragile]{Numerical lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8396249},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{03\_lists\_tuples\_and\_sets.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:orgc27e435},fragile]{Dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgc915ad3},fragile]{Operations on dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org53ec473},fragile]{Conditional statements}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgfd47bf0},fragile]{Conditional assignments}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org51f2b82},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{04\_if\_statements.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:org1f43a0a},fragile]{Loops in \texttt{Python}}
 \begin{itemize}
...
\begin{frame}[label={sec:org69cc21d},fragile]{The \texttt{while} loop \footnote{Picture credits:\href{https://www.python-course.eu/python3\_loops.php}{ Python Course}\label{orgf44a0f7}}}
 \begin{columns}
...
\begin{frame}[label={sec:orgaa9128b},fragile]{The \texttt{while-else} loop? \textsuperscript{\cref{orgf44a0f7}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org23ef64c},fragile]{\texttt{break} statement \textsuperscript{\cref{orgf44a0f7}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:orge1e8005},fragile]{\texttt{break} statement contd \textsuperscript{\cref{orgf44a0f7}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org332690c},fragile]{\texttt{for} loops}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org072a878},fragile]{Activity}
 \note{:B\_note:
}
...
\end{frame}
\begin{frame}[label={sec:org7317f28},fragile]{\texttt{continue} statement}
 \begin{itemize}
...
\begin{frame}[label={sec:org4a07bc3},fragile]{\texttt{continue} statement contd.}
 \begin{itemize}
...
\begin{frame}[label={sec:orga0e113f},fragile]{Practice}
 \begin{block}{Please attempt}
...
\begin{frame}[label={sec:orgd860aad},fragile]{Towards functions}
 \begin{itemize}
...
\begin{frame}[label={sec:org0ec1180},fragile]{Quick detour: How to format output?}
 \begin{itemize}
\item Always use the \texttt{format()} method for formatting strings
\item Positional or keyword params \textsuperscript{\cref{orgf44a0f7}}
\end{itemize}
\begin{center}
...
\begin{frame}[label={sec:org4565e7f}]{Functions}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb153833},fragile]{Skeleton of a function}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc82325c},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{07\_introduction\_to\_functions.ipynb}
\texttt{08\_some\_more\_functions.ipynb}
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdf37c21},fragile]{Call by value/reference \#1?}
...
\footnotesize
\begin{verbatim}
First call 4424577720 with value 3.14159
x = 3.14159  id = 4424577720
x = 42.0  id = 4424577384
Second call 4424577720 with value 3.14159
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:org0c43d52},fragile]{Call by value/reference \#2?}
 What's the output of the following code block?
\footnotesize
\begin{minted}[]{python}
def side_effects(x):
...
\scriptsize
\begin{verbatim}
First call 4456083272 with value ['cs', 'mechse', 'matse']
x = ['cs', 'mechse', 'matse']  id = 4456083272
x = ['cs', 'mechse', 'matse', 'aero', 'civil']  id = 4458042952
Second call 4456083272 with value ['cs', 'mechse', 'matse']
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdd57238},fragile]{Call by value/reference \#3?}
 What's the output of the following code block?
\footnotesize
\begin{minted}[]{python}
def side_effects(x):
...
\scriptsize
\begin{verbatim}
First call 4403285832 with value ['cs', 'mechse', 'matse']
x = ['cs', 'mechse', 'matse']  id = 4403285832
x = ['cs', 'mechse', 'matse', 'aero', 'civil']  id = 4403285832
Second call 4403285832 with value ['cs', 'mechse', 'matse', 'aero', 'civil']
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:orgce2b3ad}]{Call by value/reference}
\begin{alertblock}{Take-away}
Only mutable structures can be changed with in-place transformations!
\end{alertblock}
\end{frame}

\begin{frame}[label={sec:org2a20aa1}]{Recursive functions}
\note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org975c994},fragile]{Factorial done recursively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:orgb73ee76},fragile]{Factorial done iteratively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:orgd52cc61}]{Which one is preferable?}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc5f39d3},fragile]{Towers of Hanoi}
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org4cf53d2}]{Solution strategy \footnote{\href{https://www.cs.sfu.ca/\~tamaras/recursion/Rules\_Towers\_Hanoi.html}{Computing Science - Simon Fraser University}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgfb5fc48},fragile]{Towers of Hanoi}
 \footnotesize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org4d81174},fragile]{Basic I/O}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgadeafba},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org20198a8},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org37085cd},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orga4013a6},fragile]{Counting Words}
 \begin{block}{Activity}
...
\begin{frame}[label={sec:org792cb4f},fragile]{Classes : Object oriented programming}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc2e775a},fragile]{Classes : Object oriented programming}
 We have come a full circle! All variables (ints, floats, sequences) are
classes in \texttt{Python}!!!
\end{frame}
\begin{frame}[label={sec:orgfb1ed89},fragile]{Modules}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org806d5a5},fragile]{Packages}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge274669},fragile]{Why do we need numpy/ scipy/ matplotlib?}
 \begin{itemize}
...
\item The scientific computing eco-system in \texttt{Python} \textsuperscript{\cref{orgf44a0f7}}
\end{itemize}
\begin{center}
