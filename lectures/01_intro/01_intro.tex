...
\begin{frame}[label={sec:orga1f72f6}]{Logistics}
\begin{itemize}
\item Office hours
...
\label{sec:orgfbe2ae4}
\begin{frame}[label={sec:org8e25bfa}]{Last class}
\begin{columns}
...
\begin{frame}[label={sec:org8aae298},fragile]{Content}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc8188cb},fragile]{Quick survey}
 \begin{itemize}
\item \texttt{Python}?
...
\end{frame}
\section{Why python?}
\label{sec:org74d68ec}
\begin{frame}[label={sec:org4b4bf9a}]{Python is popular \footnote{Slide credits: \href{https://github.com/williamgrimes/teach\_python\_in\_notebooks/blob/master/python\_course.pdf}{William Grimes @ Github}}}
\begin{columns}
...
\begin{frame}[label={sec:org6a2c748}]{Python as a language}
\begin{itemize}
...
\item Widely used as an application/library API\footnote{\href{https://www.pluralsight.com/blog/software-development/why-python}{Pluralsight}\label{org05b4569}}
\item \(\therefore\) productivity
\end{itemize}
\end{frame}

\begin{frame}[label={sec:orgbae56d7},fragile]{Interactive\footnote{\href{https://wiki.python.org/moin/IntegratingPythonWithOtherLanguages}{Wiki Python}}}
 \begin{itemize}
...
\begin{frame}[label={sec:org0765c76}]{FlappyBird      \footnote{\href{https://yanpanlau.github.io/2016/07/10/FlappyBird-Keras.html}{ FlappyBird-Keras}, \href{https://github.com/yanpanlau/Keras-FlappyBird/blob/master/qlearn.py}{Code} , \href{https://www.pygame.org/wiki/about}{Pygame}, \href{https://scikit-image.org/}{Scikit-image}, \href{https://www.tensorflow.org/}{TensorFlow}}}
\begin{columns}
...
\begin{frame}[label={sec:org018dd71},fragile]{Comp. modeling with HPC using FOSS packages}
...
\begin{frame}[label={sec:org50b3114}]{Your favorite applications use Python!\textsuperscript{\ref{org05b4569}}}
\begin{itemize}
\item Dropbox
\item Spotify
\item Email clients
...
\begin{frame}[label={sec:org17286f0},fragile]{We will use python for\footnote{\href{https://zulko.github.io/blog/2014/11/13/things-you-can-do-with-python-and-pov-ray/\#disqus\_thread}{Vapory/Povray}}\textsuperscript{,}\,\footnote{\href{http://mattia-lab.com/work/soft-filaments/}{Mattialab}}}
 \begin{itemize}
...
\label{sec:org0d86eb3}
\begin{frame}[label={sec:orge4c766c}]{Shell?}
\begin{itemize}
...
\begin{frame}[label={sec:orgfed2d25},fragile]{On to your shell}
 \begin{itemize}
...
\begin{frame}[label={sec:org7c713e4},fragile]{Shell demo}
...
\begin{frame}[label={sec:org12b8dd4},fragile]{View current directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org9664200},fragile]{My current directory structure?}
 \begin{block}{Command}
\begin{minted}[]{fish}
...
\begin{frame}[label={sec:org016bc9c},fragile]{What's my current directory?}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:orgfaefa90},fragile]{Navigate to another directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:orgdfed68d},fragile]{Navigate back to source directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
cd images # Change Directory
...
\begin{frame}[label={sec:org26c1706},fragile]{Make a new directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org152d417},fragile]{Remove directories and files}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org534ff3a},fragile]{If the directory has files, then}
 \begin{block}{Command}
\begin{minted}[]{bash}
mkdir MyFolderName
...
\begin{frame}[label={sec:org91f6649},fragile]{See file contents}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org8580c1f},fragile]{Move files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:orgd6250e0},fragile]{Copy files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:orgbcf153f},fragile]{Launch applications}
 \begin{block}{Command}
\begin{minted}[]{bash}
...
\begin{frame}[label={sec:org86b7cbf}]{More about your shell}
\begin{itemize}
...
\label{sec:org547c0e7}
\begin{frame}[label={sec:orgbe87a52},fragile]{Outline\footnote{\url{https://www.python-course.eu/python3\_course.php}}}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf0e7539},fragile]{Installation/Setup}
 \begin{itemize}
...
\begin{frame}[label={sec:org5955bb5},fragile]{Python interactive shell}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org888890c},fragile]{Python scripts}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org33cd6b5},fragile]{Common types and operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org1e20057},fragile]{Common types and operations contd.}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org6f37954},fragile]{Some more operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org11010e1},fragile]{Practice}
...
\begin{frame}[label={sec:orgaa8293b},fragile]{Sequential types}
 \begin{itemize}
...
\begin{frame}[label={sec:org4258d44},fragile]{Lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgad67305},fragile]{Examples of lists}
 \begin{center}
\begin{tabular}{ll}
\toprule
List & Description\\
...
\begin{frame}[label={sec:orgb85c744},fragile]{Operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org7667e9c},fragile]{Additional operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org75d3da9},fragile]{Activity}
 \note{:B\_note:
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org16a1bb9},fragile]{Tuples (very briefly)}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org288a99a},fragile]{Copies of lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgd37be28},fragile]{Numerical lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org26e796b},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{03\_lists\_tuples\_and\_sets.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:org325fb8f},fragile]{Dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orga23b442},fragile]{Operations on dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgdad934f},fragile]{Conditional statements}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga388554},fragile]{Conditional assignments}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf78e566},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{04\_if\_statements.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:org0f3c8de},fragile]{Loops in \texttt{Python}}
 \begin{itemize}
...
\begin{frame}[label={sec:org1bc5d67},fragile]{The \texttt{while} loop \footnote{Picture credits:\href{https://www.python-course.eu/python3\_loops.php}{ Python Course}\label{orgbbe3d87}}}
 \begin{columns}
...
\begin{frame}[label={sec:org05753cb},fragile]{The \texttt{while-else} loop? \textsuperscript{\ref{orgbbe3d87}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\end{itemize}}
\end{frame}
\begin{frame}[label={sec:org16edd0d},fragile]{\texttt{break} statement \textsuperscript{\ref{orgbbe3d87}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org145b830},fragile]{\texttt{break} statement contd \textsuperscript{\ref{orgbbe3d87}}}
 \begin{columns}
\begin{column}{0.48\columnwidth}
\scriptsize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org22bce48},fragile]{\texttt{for} loops}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org398eb86},fragile]{Activity}
 \note{:B\_note:
}
...
\end{frame}
\begin{frame}[label={sec:org9d7ff17},fragile]{\texttt{continue} statement}
 \begin{itemize}
...
\begin{frame}[label={sec:org6695abd},fragile]{\texttt{continue} statement contd.}
 \begin{itemize}
...
\begin{frame}[label={sec:orgac7c978},fragile]{Practice}
 \begin{block}{Please attempt}
...
\begin{frame}[label={sec:orgc2b81ef},fragile]{Towards functions}
 \begin{itemize}
...
\begin{frame}[label={sec:org31e2114},fragile]{Quick detour: How to format output?}
 \begin{itemize}
\item Always use the \texttt{format()} method for formatting strings
\item Positional or keyword params \textsuperscript{\ref{orgbbe3d87}}
\end{itemize}
\begin{center}
...
\begin{frame}[label={sec:org463e304}]{Functions}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org47c7763},fragile]{Skeleton of a function}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2734f4c},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{07\_introduction\_to\_functions.ipynb}
\texttt{08\_some\_more\_functions.ipynb}
\end{block}
\end{frame}
\begin{frame}[label={sec:org89f1611},fragile]{Call by value/reference \#1?}
...
\footnotesize
\begin{verbatim}
First call 4309148344 with value 3.14159
x = 3.14159  id = 4309148344
x = 42.0  id = 4309148008
Second call 4309148344 with value 3.14159
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:org175ec32},fragile]{Call by value/reference \#2?}
 What's the output of the following code block?
\footnotesize
\begin{minted}[]{python}
def side_effects(x):
...
\scriptsize
\begin{verbatim}
First call 4476284744 with value ['cs', 'mechse', 'matse']
x = ['cs', 'mechse', 'matse']  id = 4476284744
x = ['cs', 'mechse', 'matse', 'aero', 'civil']  id = 4478247368
Second call 4476284744 with value ['cs', 'mechse', 'matse']
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:orgbddfe1c},fragile]{Call by value/reference \#3?}
 What's the output of the following code block?
\footnotesize
\begin{minted}[]{python}
def side_effects(x):
...
\scriptsize
\begin{verbatim}
First call 4560445256 with value ['cs', 'mechse', 'matse']
x = ['cs', 'mechse', 'matse']  id = 4560445256
x = ['cs', 'mechse', 'matse', 'aero', 'civil']  id = 4560445256
Second call 4560445256 with value ['cs', 'mechse', 'matse', 'aero', 'civil']
\end{verbatim}
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc1318b7}]{Call by value/reference}
\begin{alertblock}{Take-away}
Only mutable structures can be changed with in-place transformations!
\end{alertblock}
\end{frame}

\begin{frame}[label={sec:org450eaf0}]{Recursive functions}
\note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org85d79a4},fragile]{Factorial done recursively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:org552fe6d},fragile]{Factorial done iteratively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:orga355fed}]{Which one is preferable?}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org9dae5c1},fragile]{Towers of Hanoi}
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org1937934}]{Solution strategy \footnote{\href{https://www.cs.sfu.ca/\~tamaras/recursion/Rules\_Towers\_Hanoi.html}{Computing Science - Simon Fraser University}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgf28643e},fragile]{Towers of Hanoi}
 \footnotesize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:orgd097e70},fragile]{Basic I/O}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3ee36a5},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb2f99e9},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org917193e},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orgdd94072},fragile]{Counting Words}
 \begin{block}{Activity}
...
\begin{frame}[label={sec:org5761242},fragile]{Classes : Object oriented programming}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3970e5b},fragile]{Classes : Object oriented programming}
 We have come a full circle! All variables (ints, floats, sequences) are
classes in \texttt{Python}!!!
\end{frame}
\begin{frame}[label={sec:org545b6bb},fragile]{Modules}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgb2cbb4a},fragile]{Packages}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga34aaa2},fragile]{Why do we need numpy/ scipy/ matplotlib?}
 \begin{itemize}
...
\item The scientific computing eco-system in \texttt{Python} \textsuperscript{\ref{orgbbe3d87}}
\end{itemize}
\begin{center}
