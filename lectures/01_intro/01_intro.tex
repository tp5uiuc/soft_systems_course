...
\begin{frame}[label={sec:orgc6646d1}]{Logistics}
\begin{itemize}
\item Office hours
...
\label{sec:orgeb89e2f}
\begin{frame}[label={sec:org7fc28dc}]{Last class}
\begin{columns}
...
\begin{frame}[label={sec:orgef43002},fragile]{Content}
 \begin{itemize}
...
\begin{frame}[label={sec:org9eb0e30},fragile]{Quick survey}
 \begin{itemize}
\item \texttt{Python}?
...
\end{frame}
\section{Why python?}
\label{sec:org338c7e8}
\begin{frame}[label={sec:orgf73d3ef}]{Python is popular \footnote{Slide credits: \href{https://github.com/williamgrimes/teach\_python\_in\_notebooks/blob/master/python\_course.pdf}{William Grimes @ Github}}}
\begin{columns}
...
\begin{frame}[label={sec:org18cf1da}]{Python as a language}
\begin{itemize}
...
\item Widely used as an application/library API\footnote{\href{https://www.pluralsight.com/blog/software-development/why-python}{Pluralsight}\label{orge9cbbae}}
\item \(\therefore\) productivity
\end{itemize}
\end{frame}

\begin{frame}[label={sec:org98f46b8},fragile]{Interactive\footnote{\href{https://wiki.python.org/moin/IntegratingPythonWithOtherLanguages}{Wiki Python}}}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc81f054}]{FlappyBird      \footnote{\href{https://yanpanlau.github.io/2016/07/10/FlappyBird-Keras.html}{ FlappyBird-Keras}, \href{https://github.com/yanpanlau/Keras-FlappyBird/blob/master/qlearn.py}{Code} , \href{https://www.pygame.org/wiki/about}{Pygame}, \href{https://scikit-image.org/}{Scikit-image}, \href{https://www.tensorflow.org/}{TensorFlow}}}
\begin{columns}
...
\begin{frame}[label={sec:orga57b2e3},fragile]{Comp. modeling with HPC using FOSS packages}
...
\begin{frame}[label={sec:org915291b},fragile]{On to your shell}
 \begin{itemize}
...
\begin{frame}[label={sec:org23a8308},fragile]{Shell demo}
...
\begin{frame}[label={sec:orgef167ee},fragile]{View current directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:orgeb6e9c4},fragile]{My current directory structure?}
 \begin{block}{Command}
\begin{minted}[]{fish}
...
\begin{frame}[label={sec:org624367f},fragile]{What's my current directory?}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org6b912d3},fragile]{Navigate to another directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org655092c},fragile]{Navigate back to source directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
cd images # Change Directory
...
\begin{frame}[label={sec:org2470014},fragile]{Make a new directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org42a18dc},fragile]{Remove directories and files}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:orgaa36b32},fragile]{If the directory has files, then}
 \begin{block}{Command}
\begin{minted}[]{bash}
mkdir MyFolderName
...
\begin{frame}[label={sec:org8e872a8},fragile]{See file contents}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:orgbe5a3c0},fragile]{Move files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org829bad1},fragile]{Copy files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org06de639},fragile]{Launch applications}
 \begin{block}{Command}
\begin{minted}[]{bash}
...
\begin{frame}[label={sec:orgd35bcd6}]{More about your shell}
\begin{itemize}
...
\label{sec:org0ef9c6f}
\begin{frame}[label={sec:orgba43668},fragile]{Outline\footnote{\url{https://www.python-course.eu/python3\_course.php}}}
 \begin{itemize}
...
\begin{frame}[label={sec:org7f8c651},fragile]{Installation/Setup}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb59b3f7},fragile]{Python interactive shell}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc1a59af},fragile]{Python scripts}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgbc7afe9},fragile]{Common types and operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org21f0cf6},fragile]{Common types and operations contd.}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgffc17a4},fragile]{Some more operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgaeac1ba},fragile]{Practice}
...
\begin{frame}[label={sec:orga78f995},fragile]{Sequential types}
 \begin{itemize}
...
\begin{frame}[label={sec:org083ba30},fragile]{Lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5f62bde},fragile]{Examples of lists}
 \begin{center}
\begin{tabular}{ll}
\toprule
List & Description\\
...
\begin{frame}[label={sec:org1577af6},fragile]{Operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org2c224e2},fragile]{Additional operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgae0b1a8},fragile]{Activity}
 \note{:B\_note:
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orga38a53a},fragile]{Tuples (very briefly)}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org68c4da4},fragile]{Copies of lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:orgff73a65},fragile]{Numerical lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org130351c},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{03\_lists\_tuples\_and\_sets.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:orga9822c5},fragile]{Dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org21a6ae6},fragile]{Operations on dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgfbd771c},fragile]{Conditional statements}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5dc740a},fragile]{Conditional assignments}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org031f6ee},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{04\_if\_statements.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:org4cf6f08},fragile]{Loops in \texttt{Python}}
 \begin{itemize}
...
\begin{frame}[label={sec:orge409cf4},fragile]{The \texttt{while} loop \footnote{Picture credits:\href{https://www.python-course.eu/python3\_loops.php}{ Python Course}\label{orgd6b116e}}}
 \begin{columns}
...
\begin{frame}[label={sec:orgbfd3ca3},fragile]{\texttt{for} loops}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8bf542b},fragile]{Activity}
 \note{:B\_note:
}
...
\end{frame}
\begin{frame}[label={sec:org00adff9},fragile]{\texttt{continue} statement}
 \begin{itemize}
...
\begin{frame}[label={sec:org12a211f},fragile]{\texttt{continue} statement contd.}
 \begin{itemize}
...
\begin{frame}[label={sec:org14dd33c},fragile]{Practice}
 \begin{block}{Please attempt}
...
\begin{frame}[label={sec:org604d99e},fragile]{Towards functions}
 \begin{itemize}
...
\begin{frame}[label={sec:org3630159}]{Functions}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org82a7362},fragile]{Skeleton of a function}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf59ab2d},fragile]{Practice}
 \begin{block}{Please attempt}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgce6343a}]{Call by value/reference}
\begin{alertblock}{Take-away}
Only mutable structures can be changed with in-place transformations!
\end{alertblock}
\end{frame}

\begin{frame}[label={sec:org2e27d17}]{Recursive functions}
\note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org374930f},fragile]{Factorial done recursively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:org57a0bbc},fragile]{Factorial done iteratively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:orge444927}]{Which one is preferable?}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgcec552f},fragile]{Towers of Hanoi}
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org6349412}]{Solution strategy \footnote{\href{https://www.cs.sfu.ca/\~tamaras/recursion/Rules\_Towers\_Hanoi.html}{Computing Science - Simon Fraser University}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org6a1fe4e},fragile]{Towers of Hanoi}
 \footnotesize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org1c5b1b4},fragile]{Basic I/O}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org187bbcb},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf6fb91e},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org45d8584},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc99983c},fragile]{Counting Words}
 \begin{block}{Activity}
...
\begin{frame}[label={sec:orgfcfb3ae},fragile]{Classes : Object oriented programming}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8785b33},fragile]{Classes : Object oriented programming}
 We have come a full circle! All variables (ints, floats, sequences) are
classes in \texttt{Python}!!!
\end{frame}
\begin{frame}[label={sec:orge271a13},fragile]{Modules}
 \note{:B\_note:
\begin{itemize}
