...
\begin{frame}[label={sec:org0fd1bb4}]{Logistics}
\begin{itemize}
\item Office hours
...
\label{sec:org23d295a}
\begin{frame}[label={sec:org8bf4220}]{Last class}
\begin{columns}
...
\begin{frame}[label={sec:org40cd9b7},fragile]{Content}
 \begin{itemize}
...
\begin{frame}[label={sec:org94c8abe},fragile]{Quick survey}
 \begin{itemize}
\item \texttt{Python}?
...
\end{frame}
\section{Why python?}
\label{sec:org9f29e0c}
\begin{frame}[label={sec:org69402c9}]{Python is popular \footnote{Slide credits: \href{https://github.com/williamgrimes/teach\_python\_in\_notebooks/blob/master/python\_course.pdf}{William Grimes @ Github}}}
\begin{columns}
...
\begin{frame}[label={sec:org5d2eeef}]{Python as a language}
\begin{itemize}
...
\item Widely used as an application/library API\footnote{\href{https://www.pluralsight.com/blog/software-development/why-python}{Pluralsight}\label{org24613f0}}
\item \(\therefore\) productivity
\end{itemize}
\end{frame}

\begin{frame}[label={sec:org463cb87},fragile]{Interactive\footnote{\href{https://wiki.python.org/moin/IntegratingPythonWithOtherLanguages}{Wiki Python}}}
 \begin{itemize}
...
\begin{frame}[label={sec:orgdf6465a},fragile]{Comp. modeling with HPC using FOSS packages}
...
\begin{frame}[label={sec:orgcd100bf},fragile]{On to your shell}
 \begin{itemize}
...
\begin{frame}[label={sec:org06de9f5},fragile]{Shell demo}
...
\begin{frame}[label={sec:org05d497d},fragile]{View current directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org4f7c888},fragile]{My current directory structure?}
 \begin{block}{Command}
\begin{minted}[]{fish}
...
\begin{frame}[label={sec:orgf305e9a},fragile]{What's my current directory?}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org6d15aab},fragile]{Navigate to another directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org3dbb81e},fragile]{Navigate back to source directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
cd images # Change Directory
...
\begin{frame}[label={sec:org6c121a7},fragile]{Make a new directory}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org8e310f3},fragile]{Remove directories and files}
 \begin{block}{Command}
\begin{minted}[]{shell}
...
\begin{frame}[label={sec:org038c1e0},fragile]{If the directory has files, then}
 \begin{block}{Command}
\begin{minted}[]{bash}
mkdir MyFolderName
...
\begin{frame}[label={sec:org3f693da},fragile]{See file contents}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org3b4478e},fragile]{Move files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org05a88a7},fragile]{Copy files around}
 \begin{block}{Command}
\begin{minted}[]{sh}
...
\begin{frame}[label={sec:org15b4ae9},fragile]{Launch applications}
 \begin{block}{Command}
\begin{minted}[]{bash}
...
\begin{frame}[label={sec:org2967000}]{More about your shell}
\begin{itemize}
...
\label{sec:org658f044}
\begin{frame}[label={sec:org37defd1},fragile]{Outline\footnote{\url{https://www.python-course.eu/python3\_course.php}}}
 \begin{itemize}
...
\begin{frame}[label={sec:org2e5f59c},fragile]{Installation/Setup}
 \begin{itemize}
...
\begin{frame}[label={sec:org977547e},fragile]{Python interactive shell}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgbe3dd75},fragile]{Python scripts}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org8d7896a},fragile]{Common types and operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org683db81},fragile]{Common types and operations contd.}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org958ce96},fragile]{Some more operations}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org7933f18},fragile]{Practice}
...
\begin{frame}[label={sec:orgc6b68de},fragile]{Sequential types}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf380dcc},fragile]{Lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org19bfb92},fragile]{Examples of lists}
 \begin{center}
\begin{tabular}{ll}
\toprule
List & Description\\
...
\begin{frame}[label={sec:org4dd18da},fragile]{Operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org4a01597},fragile]{Additional operations on lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgadef4d3},fragile]{Activity}
 \note{:B\_note:
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org1f37fc7},fragile]{Tuples (very briefly)}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge9f7319},fragile]{Copies of lists}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org64ba393},fragile]{Numerical lists}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb5aec36},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{03\_lists\_tuples\_and\_sets.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:orge4b80a8},fragile]{Dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org46cc2a4},fragile]{Operations on dictionaries}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3dca4f5},fragile]{Conditional statements}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6bf2aca},fragile]{Conditional assignments}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5a69800},fragile]{Practice}
 \begin{block}{Please attempt}
\texttt{04\_if\_statements.ipynb}
\end{block}
\end{frame}

\begin{frame}[label={sec:orgfdd0bb2},fragile]{Loops in \texttt{Python}}
 \begin{itemize}
...
\begin{frame}[label={sec:org5c341fb},fragile]{The \texttt{while} loop \footnote{Picture credits:\href{https://www.python-course.eu/python3\_loops.php}{ Python Course}\label{org970cafa}}}
 \begin{columns}
...
\begin{frame}[label={sec:org13753d0},fragile]{\texttt{for} loops}
 \note{:B\_note:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgdfb23e1},fragile]{Activity}
 \note{:B\_note:
}
...
\end{frame}
\begin{frame}[label={sec:orgd7bb230},fragile]{\texttt{continue} statement}
 \begin{itemize}
...
\begin{frame}[label={sec:org972687b},fragile]{\texttt{continue} statement contd.}
 \begin{itemize}
...
\begin{frame}[label={sec:org8ecdb3c},fragile]{Practice}
 \begin{block}{Please attempt}
...
\begin{frame}[label={sec:orga820198},fragile]{Towards functions}
 \begin{itemize}
...
\begin{frame}[label={sec:org200831b}]{Functions}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0c3b984},fragile]{Skeleton of a function}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org54e46ca},fragile]{Practice}
 \begin{block}{Please attempt}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orged166bf}]{Call by value/reference}
\begin{alertblock}{Take-away}
Only mutable structures can be changed with in-place transformations!
\end{alertblock}
\end{frame}

\begin{frame}[label={sec:org7aa2852}]{Recursive functions}
\note{:B\_note:
\begin{itemize}
...
\begin{frame}[label={sec:org85e6c85},fragile]{Factorial done recursively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:org80b7a39},fragile]{Factorial done iteratively}
 \begin{equation*}
 n! = n \times (n-1) \times (n-2) \times \cdots \times 1
\end{equation*}
...
\begin{frame}[label={sec:org6d76ada}]{Which one is preferable?}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org1581075},fragile]{Towers of Hanoi}
...
\end{figure}
\end{frame}
\begin{frame}[label={sec:org9accea9}]{Solution strategy \footnote{\href{https://www.cs.sfu.ca/\~tamaras/recursion/Rules\_Towers\_Hanoi.html}{Computing Science - Simon Fraser University}}}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org3a338be},fragile]{Towers of Hanoi}
 \footnotesize
\begin{minted}[]{python}
...
\begin{frame}[label={sec:org54a04aa},fragile]{Basic I/O}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb27a314},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org47b8726},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:org6e79fb7},fragile]{Basic I/O continued}
 \begin{itemize}
...
\begin{frame}[label={sec:orge42dbaa},fragile]{Counting Words}
 \begin{block}{Activity}
...
\begin{frame}[label={sec:org31fccca},fragile]{Classes : Object oriented programming}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgfa97b80},fragile]{Classes : Object oriented programming}
 We have come a full circle! All variables (ints, floats, sequences) are
classes in \texttt{Python}!!!
\end{frame}
\begin{frame}[label={sec:org02092d3},fragile]{Modules}
 \note{:B\_note:
\begin{itemize}
